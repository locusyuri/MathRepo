...
3. 全部完成后：编译通过 → 按 `.trae/rules/git-commit-message.md` 规范提交（中文 Conventional Commits，**只提交自己修改的文件**，工作区中他人的未提交改动不得夹带）。
4. 输出修改汇总：
...
- **承诺-兑现**：文中所有"proof deferred to …""to be proved in …"的承诺，是否在承诺位置真正兑现（逐一核对，这是最容易遗漏的一项）；
...
- **双语规范**：正文不得出现中文；标题带 `// 中文注释`；
- **数学语法**：符合 `typst-writing-conventions`（符号映射、`abs()` 写法、分数括号、dif、display 公式标点在环境内等）。
