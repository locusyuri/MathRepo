...
$2.1^\circ$ 作辅助函数
对于任意固定的$x\in[a,b]$,定义函数列$\psi_{n}(t)=\begin{cases} \frac{f_{n}(t)-f_{n}(x)}{t-x},&t\neq x\\f'_{n}(x),&t=x\end{cases}$
类似定义函数$\psi(t)=\begin{cases} \frac{f(t)-f(x)}{t-x},&t\neq x\\\sigma(x),&t=x\end{cases}$
$2.2^\circ$ $\psi_{n}(t)$在包含$x$的闭区间上一致收敛于$\psi(t)$
由于$f'_{n}(x)\mathop{\rightrightarrows}\limits^{[a,b]}\sigma(x)$,$\forall\varepsilon>0,\exists N,\forall n>N:|f'_{n}(x)-\sigma(x)|<\varepsilon\quad(\forall x\in[a,b])$
当$t\neq x$时,由原函数列的一致收敛性,
$|\psi_{n}(t)-\psi(t)|=| \frac{[f_{n}(t)-f(t)]-[f_{n}(x)-f(x)]}{t-x}|\leqslant|f'_{n}(\xi)-\sigma(\xi)|<\varepsilon$($\xi$在$t,x$之间)
当$t=x$时,由导函数列的一致收敛性,直接有:
$|\psi_{n}(x)-\psi(x)|=|f'_{n}(x)-\sigma(x)|<\varepsilon$
因此$\psi_{n}(t)$在包含$x$的闭区间上一致收敛于$\psi(t)$,进而$\psi(t)$在$t=x$处连续
$2.3^\circ$ $f'(x)=\sigma(x)$
由于$\psi(t)$在$t=x$处连续,因此极限运算可以交换:
...
>则幂级数当$|x|<R$时绝对收敛;当$|x|>R$时发散;在端点处的情况必须单独判断
...
>设幂级数$\sum\limits_{n=0}^{\infty}a_{n}x^n$收敛半径为1,且$\lim_\limits{ x \to 1^-}\sum\limits_{n=0}^{\infty}a_{n}x^n=A\in \mathbb{R}$.如果$a_{n}=o\left( \frac{1}{n} \right)(n\to+\infty)$,则$\sum\limits_{n=0}^{\infty}a_{n}=\sum\limits_{n=0}^{\infty}a_{n}\cdot1^n=A$
>
>如果使$a_{n}\geqslant0$,Abel第二定理逆命题成立
><span style='font-family:汉仪劲楷简 Regular,Stem;font-style: normal;font-weight: bold;color: #242424;text-decoration: none;text-shadow: 1px 1px 1px #eeece1;font-size:1.5em;background-image: linear-gradient(to right, #92fe9d 0%, #00c9ff 100%);border-radius:7px;box-shadow: 1px 1px 29px rgb(204, 204, 204, 0.68);padding:3px;'>Theorem</span>
>设幂级数$\sum\limits_{n=0}^{\infty}a_{n}x^n$收敛半径为1,且$\lim_\limits{ x \to 1^-}\sum\limits_{n=0}^{\infty}a_{n}x^n=A\in \mathbb{R}$.如果$a_{n}\geqslant0,n=0,1,2,\dots$,则$\sum\limits_{n=0}^{\infty}a_{n}=\sum\limits_{n=0}^{\infty}a_{n}\cdot1^n=A$
...
>
>$1-x=\sum\limits_{n=0}^{\infty}x^n=1+x+x^2+\dots,x\in(-1,1)$
