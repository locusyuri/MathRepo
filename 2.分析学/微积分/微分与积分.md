...
设$f_{n}(x)$是定义在$[a,b]$上的一列函数$(n=0,1,2,\dots)$,若对于任意的$m,n$,$f_{m}(x),f_{n}(x)$在$[a,b]$上可积,且$\displaystyle\int_{a}^{b} f_{m}(x)f_{n}(x) \, \mathrm{d}x=\begin{cases}0,&m\neq n\\\displaystyle\int_{a}^{b} f_{n}^2(x) \, \mathrm{d}x>0,&m=n\end{cases}$,则称$\{ f_{n}(x) \}$是$[a,b]$上的正交函数列.
证明函数$\{ 1,\sin x,\cos x,\sin2x,\cos2x,\dots\sin nx,\cos nx,\dots \}$是任一长度为$2\pi$的区间上的正交函数列
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 1 </span>
讨论敛散性:
1)$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x} \, \mathrm{d}x$
2)$\displaystyle\int_{1}^{+\infty} \frac{\sin x\arctan x}{x} \, \mathrm{d}x$
3)$\displaystyle\int_{0}^{ \frac{1}{e}} \frac{1}{x^p\ln x} \, \mathrm{d}x,p\in \mathbb{R}^+$
4)$\displaystyle\int_{0}^{1} \frac{1}{x^p}\sin \frac{1}{x} \, \mathrm{d}x$
5)$\displaystyle\int_{0}^{+\infty} \frac{x^{1-p}}{\lvert x-1 \rvert^{p+q}} \, \mathrm{d}x,(p,q\in \mathbb{R})$
...
#### 3.3.1 特殊定积分
==证明Dirichlet积分==
$\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+ \frac{1}{2} \right)x}{\sin \frac{x}{2}} \, \mathrm{d}x=\pi,\quad n=0,1,2,\dots$
