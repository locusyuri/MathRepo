...
><span style='font-family:汉仪劲楷简 Regular,Stem;font-style: normal;font-weight: bold;color: #242424;text-decoration: none;text-shadow: 1px 1px 1px #eeece1;font-size:1.5em;background-image: linear-gradient(to right, #92fe9d 0%, #00c9ff 100%);border-radius:7px;box-shadow: 1px 1px 29px rgb(204, 204, 204, 0.68);padding:3px;'>Theorem:Chebyshev定理</span>
><span style="background:#d2cbff">二项微分式</span>的积分$\displaystyle\int x^m(a+bx^n)^p\mathrm{d}x$(其中$m,n,p\in \mathbb{Q},a,b\in \mathbb{R}$)仅在以下情况可化为有理函数的积分(即可求不定积分):
>1)$p\in \mathbb{Z}$,此时令$x=t^N$,其中$N$为分数$m,n$的公分母
>2)$\frac{m+1}{n}\in \mathbb{Z}$,此时令$a+bx^n=t^N$,其中$N$为分数$p$的分母
>3)$\frac{m+1}{n}+p\in \mathbb{Z}$,此时令$ax^{-n}+b=t^N$,其中$N$为分数$p$的分母
...
#### 2.1.3 Riemann-Stieltjes积分
> <span style='background-image: linear-gradient(120deg, #89f7fe 0%, #66a6ff 100%);font-size:1.5em;font-style:normal;padding:2px;font-family:方正龙爪简体;font-weight:bold;border-radius:0 10px 0 0;box-shadow: 2px 2px 5px 3px #ccc;'>「Definition：Riemann-Stieltjes积分」</span>
> 设$\alpha$是$[a,b]$上一个单调递增的有界函数,对应$[a,b]$的每个分法P,记$\Delta\alpha_{i}=\alpha(x_{i})-\alpha(x_{i-1})$(显然$\Delta\alpha_{i}\geqslant0$)
> 对于$[a,b]$上的有界实函数$f(x)$,作Stieltjes大小和:$\bar{S}(P,\alpha)=\sum\limits_{i=1}^{n}M_{i}\alpha_{i},\underline{S}(P,\alpha)=\sum\limits_{i=1}^{n}m_{i}\alpha_{i}$,且定义上下积分:$L=\inf\{ \bar{S}(P,\alpha)|\bar{S}(P,\alpha)\in \boldsymbol{\bar{S}} \},l=\sup\{ \underline{S}(P,\alpha)|\underline{S}(P,\alpha)\in \boldsymbol{\underline{S}} \}$,其中$\boldsymbol{\bar{S},\underline{S}}$分别是Stieltjes大小和的集合
> 若$L=l$,则记$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}\alpha(x)=L=l$,称为$f(x)$在$[a,b]$上关于$\alpha$的Riemann-Stieltjes积分,或简称Stieltjes积分
> 
> 当$\alpha(x)=x$时,即为Riemann积分,但是在一般情形,$\alpha(x)$<span style="background:#d2cbff">甚至不一定连续</span>
> Darboux和的性质对于Stieltjes和同样成立

#### 2.1.4 Riemann可积的充要条件
...
**Corollary-1**
根据连续得一致连续,由一致连续可估计$\omega_{i}=\max\limits_{x\in[x_{i-1},x_{i}]} f(x)-\min\limits_{x\in[x_{i-1},x_{i}]} f(x)< \frac{\varepsilon}{b-a}$
因此$\sum\limits_{i=1}^{n}\omega_{i}\Delta x_{i}<\varepsilon$

**Corollary-2**
由单调得$\omega_{i}=f(x_{i})-f(x_{i-1})$,又有$\Delta x_{i}\leqslant\delta= \frac{\varepsilon}{f(b)-f(a)}$
因此$\sum\limits_{i=1}^{n}\omega_{i}\Delta x_{i}<\varepsilon$
...
由第三充要条件即证
$\Longrightarrow$
由第三充要条件,$\forall \varepsilon,\eta>0,\exists P:\sum\limits_{i=1}^{n}\omega_{i}\Delta x_{i}<\varepsilon\eta$
因此有:
...
**Corollary-2**
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311817760.png)
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 1 </span>
证明Riemann函数在$[0,1]$上可积$R(x)=\begin{cases} \frac{1}{q},&x=\frac{p}{q}\\0,&x\in \mathbb{J}\end{cases}$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
$\forall\varepsilon>0$,令$R(x)=\frac{1}{q}\geqslant \frac{\varepsilon}{2}$,即$q\leqslant \frac{2}{\varepsilon}$,显然使得$R(x)> \frac{\varepsilon}{2}$的点至多有有限个,记为$k$个
取$\delta=\frac{\varepsilon}{4k}$,作分划$P(\lambda=\max\limits_{1\leqslant i\leqslant n}\Delta x_{i}<\delta)$后,令$\sum\omega_{i}\Delta x_{i}=\sum'\omega_{i}\Delta x_{i}+\sum''\omega_{i}\Delta x_{i}$
($\sum'$表示含有上述有限个点的小区间对应项之和,$\sum''$表示不含)
则
$\begin{align}\sum\omega_{i}\Delta x_{i}&=\sum'\omega_{i}\Delta x_{i}+\sum''\omega_{i}\Delta x_{i}\\&\leqslant\sum'\Delta x_{i}+\frac{\varepsilon}{2}\sum''\Delta x_{i}\\&\leqslant2k\lambda+ \frac{\varepsilon}{2}\cdot1<\varepsilon\end{align}$
<span style='display: inline-block; width: 0.8em; height: 0.8em; background: #000; margin-left: 0.5em; vertical-align: middle; box-shadow: 1px 1px 2px rgba(0,0,0,0.2); margin-left:90%'></span>
...
设$f(x)$在$[a,b]$上每一点处的极限存在且皆为零,试证:$f(x)\in R[a,b]$,且$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x=0$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
设$x_{0}\in[a,b]$中任一点,由于$\lim_\limits{ x \to  x_0 }f(x)=0$,故$\forall\varepsilon_{1}>0,\exists\delta_{x_{0}}>0,\forall x\in \mathring{U}(x_{0},\delta_{x_{0}}):\lvert f(x) \rvert<\varepsilon_{1}$
如此,开集族$\bigcup \limits_{x_{0}\in[a,b]}\mathring{U}(x_{0},\delta_{x_{0}})$组成了$[a,b]$的一个开覆盖,由有限覆盖原理,存在有限子覆盖$\bigcap \limits_{i=1}^r \mathring{U}(x_{i},\delta_{x_{i}})$
因此,除了有限个点$x_{1},x_{2},\dots x_{r}$外恒有$\lvert f(x) \rvert<\varepsilon$
故$\forall\varepsilon>0,$令$\varepsilon_{1}=\frac{\varepsilon}{4(b-a)}$,有$\lvert f(x) \rvert<\varepsilon\quad(x\neq x_{1},\dots x_{r})$,取$M>\max\{ f(x_{1}),\dots f(x_{r}),\varepsilon_{1} \}$,作划分$P\left( \sum'\Delta x_{i}< \frac{\varepsilon}{4M} \right)$,则
($\sum'$表示含有$x_{1},\dots x_{r}$的小区间对应项之和,$\sum''$表示不含)
$\sum\omega_{i}\Delta x_{i}=\sum'\omega_{i}\Delta x_{i}+\sum''\omega_{i}\Delta x_{i}\leqslant 2M\frac{\varepsilon}{4M}+2\varepsilon_{1}(b-a)=\varepsilon$
可积性得证
又由于$\forall\varepsilon>0$,至多有限个点$\lvert f(x) \rvert\geqslant\varepsilon$,可直接证明$\lim_\limits{ \lambda \to 0 }\sum\limits_{i=1}^{n}f(\xi_{i})\Delta x_{i}=0$
<span style='display: inline-block; width: 0.8em; height: 0.8em; background: #000; margin-left: 0.5em; vertical-align: middle; box-shadow: 1px 1px 2px rgba(0,0,0,0.2); margin-left:90%'></span>

<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg.  </span>
设$f(x)\in R[a,b]$,试证:$f(x)$的连续点在$[a,b]$中稠密

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311128915.png)
...
$1^\circ$ 对于$\sum_{1}$中的项,由于$K\cap(x_{i-1},x_{i})\neq \phi$,任取$y_{i}\in K\cap(x_{i-1},x_{i})$,有:
...
$2^\circ$ 对于$\sum_{2}$中的项,由于$\omega_{i}\leqslant\omega$,于是$\sum_{2}\omega_{i}\Delta x_{i}\leqslant\omega\sum_{2}\Delta x_{i}$
...
>**Corollary-1** 设$f(x)\in C[a,b],f(x)\geqslant0,f(x)\not\equiv0$,那么$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x>0$
>**Corollary-2** 设$f(x)\in R[a,b],f(x)>0$,那么$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x>0$
> 
...
事实上,$\forall[c,d]\subset[x_{i-1},x_{i}]$,有$\displaystyle\int_{c}^{d} f(x) \, \mathrm{d}x=0$
...
**Corollary-2**
由于$f(x)>0$,故$\sum\limits_{i=1}^{n}f(\xi_{i})\Delta x_{i}>0$,故$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x=\lim_\limits{ \lambda \to 0 }\sum\limits_{i=1}^{n}f(\xi_{i})\Delta x_{i}\geqslant0$
下面用反证法证明不等于0
假设$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x=0$,则对于其Darboux大和,有$\lim_\limits{ \lambda \to 0}\sum\limits_{i=1}^{n}M_{i}\Delta x_{i}=0$
从而$\forall\varepsilon_{1}>0,\exists P:\sum\limits_{i=1}^{n}M_{i}\Delta x_{i}<\varepsilon_{1}(b-a)$
由此至少存在一个$M_{i}<\varepsilon_{1}$,否则每个$M_{i}\geqslant\varepsilon_{1}$,应有$\sum\limits_{i=1}^{n}M_{i}\Delta x_{i}\geqslant\varepsilon_{1}\sum\limits_{i=1}^{n}\Delta x_{i}=\varepsilon_{1}(b-a)$,与上式矛盾
将$M_{i}<\varepsilon_{1}$的这个小区间记为$[a_{1},b_{1}]$,于是$f(x)\in R[a_{1},b_{1}]$
把$[a_{1},b_{1}]$取作上面的$[a,b]$,重复上述推理,得到$[a_{2},b_{2}]\subset[a_{1},b_{1}],s.t.\sup\limits_{a_{2}\leqslant x\leqslant b_{2}}f(x)\leqslant\varepsilon_{2}$
如此进行下去得到一列区间$[a,b]\supset[a_{1},b_{2}]\supset\dots[a_{n},b_{n}]\supset\dots$,使得每个$[a_{n},b_{n}]$上,$\sup\limits_{a_{n}\leqslant x\leqslant b_{n}}f(x)\leqslant\varepsilon_{n}(n=1,2,\dots)$
由闭区间套定理,$\exists! \xi\in[a_{n},b_{n}](n=1,2,\dots):0\leqslant f(\xi)<\varepsilon_{n}(n=1,2,\dots)$
令$\varepsilon_{n}\to0$,可知$f(\xi)=0$,矛盾
...
>
>7)(**积分第二中值定理/Bonnet公式**)设$f(x)\in R[a,b]$,
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 1 </span>
==Riemann积分在L1范数下平移连续==
证明Riemann积分的连续性:设$f(x),f(x+h)\in R[a,b]$,则有:$\lim_\limits{ h \to  0 }\displaystyle\int_{a}^{b} |f(x+h)-f(x)| \, \mathrm{d}x=0$
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 2 </span>
利用Bonnet公式证明:
1)设$f:[a,b]\to[0,1]$可导,$f'(x)$在$[a,b]$上单调下降,$f'(b)>0$.求证:$\displaystyle\int_{a}^{b} \cos f(x) \, \mathrm{d}x\leqslant \frac{2}{f'(b)}$
2)设$x>0,c>0$,试证:$\left\lvert  \displaystyle\int_{x}^{x+c} \sin t^2 \, \mathrm{d}x  \right\rvert< \frac{1}{x}$
3)设$e^2<a<b$,试证:$\displaystyle\int_{a}^{b} \frac{1}{\ln x} \, \mathrm{d}x< \frac{2b}{\ln b}$
4)设$x\neq0,f(x)=\displaystyle\int_{0}^{x} \sin \frac{1}{t} \, \mathrm{d}t,f(0)=0$,试证:$f'(0)=0$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
1)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311154699.png)

2)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311154734.png)

3)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311157757.png)

4)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311158767.png)

#### 2.2.2 复合函数的可积性
><span style='font-family:汉仪劲楷简 Regular,Stem;font-style: normal;font-weight: bold;color: #242424;text-decoration: none;text-shadow: 1px 1px 1px #eeece1;font-size:1.5em;background-image: linear-gradient(to right, #92fe9d 0%, #00c9ff 100%);border-radius:7px;box-shadow: 1px 1px 29px rgb(204, 204, 204, 0.68);padding:3px;'>Theorem:复合函数的可积性问题</span>
>==外层连续,内层可积== 设$f(x)\in R[a,b],A\leqslant f(x)\leqslant B,g(u)\in C[A,B]$,则复合函数$g(f(x))\in R[a,b]$
>==外层可积,内层连续== 此时不一定可积
>==内外均可积== 此时不一定可积,事实上内外均不可积复合函数也有可能可积
...
设$A$为区间$[0,1]$中具有正测度的Cantor集,$(a_{i},b_{i})(i=1,2,\dots)$为$A$的邻接区间.在$[0,1]$上定义:
$f(x)=\begin{cases}0,&0\leqslant x<1\\1,&x=1\end{cases}$
$g(x)=\begin{cases}1,&x\in A\\1- \frac{1}{2}(b_{i}-a_{i})+\left\lvert  x- \frac{1}{2}(a_{i}+b_{i})  \right\rvert,&x\in(a_{i},b_{i}),i=1,2,\dots\end{cases}$

则$f\in R[0,1],g\in C[0,1]$($g$满足Lipschitz条件)
然而,$f(g(x))=\begin{cases}1,&x\in A\\0,&x\in[0,1]\setminus A\end{cases}$在$A$上无处连续,而$mA>0$,因此不可积
...
1)$[0,1]$上,令$f(x)$为Riemann函数,$g(y)=\begin{cases}1,&0<y\leqslant1\\0,&y=0\end{cases}$,它们在$[0,1]$上均可积
复合函数$g(f(x))=\begin{cases}1,&x\in \mathbb{Q}\\0,&x\in \mathbb{J}\end{cases}$不可积
2)令$f,g$均为Dirichlet函数不可积,复合函数为$g(f(x))\equiv0$显然可积
...
>[!example] 
>==有原函数但不可积==
>设$f(x)=\begin{cases}x^2\sin \frac{1}{x^2},&x\neq0\\0,&x=0\end{cases}$,则$f$在闭区间$[-1,1]$的每一点都有有限导数,$g(x)=f'(x)=\begin{cases}2x\sin \frac{1}{x}- \frac{2}{x^2}\cos \frac{1}{x^2},&x\neq0\\0,&x=0\end{cases}$
>但是$g(x)$在$[-1,1]$上无界,因而在上面不可积 
...
$\frac{\Delta F}{\Delta x}=\frac{F(x+\Delta x)-F(x)}{\Delta x}=\frac{1}{\Delta x} \displaystyle\int_{x}^{x+\Delta x} g(t) \, \mathrm{d}t$
...
>
><span style='font-family:汉仪劲楷简 Regular,Stem;font-style: normal;font-weight: bold;color: #242424;text-decoration: none;text-shadow: 1px 1px 1px #eeece1;font-size:1.5em;background-image: linear-gradient(to right, #92fe9d 0%, #00c9ff 100%);border-radius:7px;box-shadow: 1px 1px 29px rgb(204, 204, 204, 0.68);padding:3px;'>Corollary</span>
>设$f(x)\in R[a,b]$,则$\lim_\limits{ p \to +\infty }\displaystyle\int_{a}^{b} f(x)\sin(px) \, \mathrm{d}x= \displaystyle\int_{a}^{b} f(x)\cos (px) \, \mathrm{d}x=0$
...
设$f(x)$在$[0,1]$上有二阶连续导数,通过缩放被积函数或积分区间进行证明:
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
1)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311133573.png)

2)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311135561.png)

3)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311135931.png)


4)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311137928.png)


<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. (A2) </span>
设$f(x)\in D^{(2)}_{[a,b]}$,利用Taylor公式证明:
1)$\exists \xi\in(a,b),s.t.f''(\xi)= \frac{24}{(b-a)^3}\displaystyle\int_{a}^{b} \left( f(x)-f\left( \frac{a+b}{2} \right) \right) \, \mathrm{d}x$
2)若$f(a)=f(b)=0,\exists \xi\in(a,b),s.t.\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x= \frac{1}{12} f''(\xi)(a-b)^3$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
1)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311142914.png)

2)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311139466.png)

<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. (B1) </span>
设$f(x)\in D[0,1]$,且当$x\in(0,1)$时,$0<f'(x)<1,f(0)=0$,试证:$\left( \displaystyle\int_{0}^{1} f(x) \, \mathrm{d}x \right)^2>\displaystyle\int_{0}^{1} f^3(x) \, \mathrm{d}x$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311141002.png)
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311141244.png)
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311143071.png)
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311144328.png)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311144189.png)
...
设$f(x)\nearrow\in C[a,b]$,则$\displaystyle\int_{a}^{b} xf(x) \, \mathrm{d}x\geqslant \frac{a+b}{2}\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311147817.png)
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311152654.png)
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 1 </span>
计算:
1)$I=\displaystyle\int_{0}^{1} \frac{\ln(1+x)}{1+x^2} \, \mathrm{d}x$
2)$\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin^2 x}{\sin x+\cos x} \, \mathrm{d}x$
3)$\displaystyle\int_{0}^{2} \frac{(x-1)^2+1}{(x-1)^2+x^2(x-2)^2} \, \mathrm{d}x$
4)$\displaystyle\int_{0}^{1} \frac{\ln x}{1-x^2} \, \mathrm{d}x$
5)$\displaystyle\int_{0}^{\frac{\pi}{2}} \sin x\ln \sin x \, \mathrm{d}x$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
1)
$\begin{align}I& \xlongequal{x=\tan t}\displaystyle\int_{0}^{\frac{\pi}{4}} \ln(1+\tan t) \, \mathrm{d}t=\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \frac{\sin t+\cos t}{\cos t} \, \mathrm{d}t=\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \frac{\sqrt{ 2 }\cos\left( \frac{\pi}{4}-t \right)}{\cos t} \, \mathrm{d}t\\&=\displaystyle\int_{0}^{\frac{\pi}{4}} \ln\sqrt{ 2 } \, \mathrm{d}t+\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos\left( \frac{\pi}{4}-t \right) \, \mathrm{d}t-\displaystyle\int_{0}^{\frac{\pi}{4}} \ln\cos t \, \mathrm{d}t\end{align}$  
又有:
$\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos\left( \frac{\pi}{4}-t \right) \, \mathrm{d}t \xlongequal{u=\frac{\pi}{4}-t}\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos u \, \mathrm{d}u$ 
因此,$I=\displaystyle\int_{0}^{\frac{\pi}{4}} \ln\sqrt{ 2 } \, \mathrm{d}t+\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos u \, \mathrm{d}u-\displaystyle\int_{0}^{\frac{\pi}{4}} \ln\cos t \, \mathrm{d}t=\frac{\pi}{8}\ln2$ 
2)
$\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin^2 x}{\sin x+\cos x} \, \mathrm{d}x \xlongequal{x=\frac{\pi}{2}-t}\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin^2 t}{\sin t+\cos t} \, \mathrm{d}t$
因此:
$\begin{align}\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin^2 x}{\sin x+\cos x} \, \mathrm{d}x&= \frac{1}{2}\left( \displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin^2 x}{\sin x+\cos x} \, \mathrm{d}x+\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\cos^2 x}{\sin x+\cos x} \, \mathrm{d}x \right)\\&= \frac{1}{2}\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{1}{\sin x+\cos x} \, \mathrm{d}x\\&=\frac{1}{2}\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{1}{\sin \left( x+\frac{\pi}{4} \right)} \, \mathrm{d}x\\&= \frac{1}{\sqrt{ 2 }}\ln(1+\sqrt{ 2 })\end{align}$
3)
令$f(x)= \frac{(x-1)^2+1}{(x-1)^2+x^2(x-2)^2}$,则$F_{1}(x)=\begin{cases}\arctan \frac{x(x-2)}{x-1},&x\in[0,1)\\ \frac{\pi}{2},&x=1\end{cases}$是$f(x)$在$[0,1]$上的原函数,$F_{2}(x)=\begin{cases}\arctan \frac{x(x-2)}{x-1},&x\in(1,2]\\ -\frac{\pi}{2},&x=1\end{cases}$是$f(x)$在$[1,2]$上的原函数
利用可分可加性,在$[0,1],[1,2]$上分别使用Newton-Leibniz公式,有:
$\begin{align}\displaystyle\int_{0}^{2} \frac{(x-1)^2+1}{(x-1)^2+x^2(x-2)^2} \, \mathrm{d}x&=\displaystyle\int_{0}^{1} \frac{(x-1)^2+1}{(x-1)^2+x^2(x-2)^2} \, \mathrm{d}x+\displaystyle\int_{1}^{2} \frac{(x-1)^2+1}{(x-1)^2+x^2(x-2)^2} \, \mathrm{d}x\\&=F_{1}(x)\Big{|}_{0}^1+F_{2}(x)\Big{|}_{1}^2=\pi\end{align}$
4)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311750125.png)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311750802.png)

5)
由于$\sin x\ln \sin x=O(1)\quad(x\to0^+)$,因而为常义积分
$\begin{align}I&=\displaystyle\int_{0}^{\frac{\pi}{2}}\ln \sin x \, \mathrm{d}(1-\cos x)\\&=(1-\cos x)\ln \sin x\Big{|}_{0^+}^{\frac{\pi}{2}}-\displaystyle\int_{0}^{\frac{\pi}{2}} (1-\cos x) \, \mathrm{d}(\ln \sin x)\\&=-\displaystyle\int_{0}^{\frac{\pi}{2}} (1-\cos x) \frac{\sin x}{\cos x} \, \mathrm{d}x\\&=-\displaystyle\int_{0}^{\frac{\pi}{2}} \frac{\sin x\cos x}{1+\cos x} \, \mathrm{d}x\\&=\displaystyle\int_{0}^{\frac{\pi}{2}} \left( -\sin x+ \frac{\sin x}{1+\cos x} \right) \, \mathrm{d}x\\&=[\cos x-\ln(1+\cos x)]\Big{|}_{0}^{\frac{\pi}{2}}\\&=\ln2-1\end{align}$


>[!caution] 
>对于3),不能将$\arctan \frac{x(x-2)}{x-1}$视为其原函数然后使用Newton-Leibniz公式求解,因为$\arctan \frac{x(x-2)}{x-1}$在$[0,2]$上有间断点$x=1$,不可能是其原函数.应当利用积分的区间可加性,分成$[0,1],[1,2]$两段再使用Newton-Leibniz公式
>Cauchy也给出了一个类似的例子:$\displaystyle\int_{0}^{\frac{3\pi}{4}} \frac{\sin x}{1+\cos^2x} \, \mathrm{d}x$

<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 4 </span>
1)设$f_{n}(x)$是定义在$[a,b]$上的一列函数$(n=0,1,2,\dots)$,若对于任意的$m,n$,$f_{m}(x),f_{n}(x)$在$[a,b]$上可积,且$\displaystyle\int_{a}^{b} f_{m}(x)f_{n}(x) \, \mathrm{d}x=\begin{cases}0,&m\neq n\\\displaystyle\int_{a}^{b} f_{n}^2(x) \, \mathrm{d}x>0,&m=n\end{cases}$,则称$\{ f_{n}(x) \}$是$[a,b]$上的正交函数列.
证明:函数$\{ 1,\sin x,\cos x,\sin2x,\cos2x,\dots\sin nx,\cos nx,\dots \}$是任一长度为$2\pi$的区间上的正交函数列

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
将$1$记为$\cos0x$,则对于任意$m=1,2,\dots;n-=0,1,2,\dots$,由于$\sin mx\cos nx$为奇函数,所以:$\displaystyle\int_{-\pi}^{\pi} \sin mx\cos nx \, \mathrm{d}x=0$
其次有对于任意$m=1,2,\dots;n-=0,1,2,\dots$,由于$\sin mx\sin nx$为偶函数,
$\begin{align}\displaystyle\int_{-\pi}^{\pi} \sin mx\sin nx \, \mathrm{d}x&=2\displaystyle\int_{0}^{\pi} \sin mx\sin nx \, \mathrm{d}x\\&=\displaystyle\int_{0}^{\pi} [\cos(m-n)x-\cos(m+n)x] \, \mathrm{d}x\\&=\begin{cases}\left[  \frac{\sin(m-n)x}{m-n}- \frac{\sin(m+n)x}{m+n} \right]\Big{|}_{0}^\pi,&m\neq n\\\left( x- \frac{\sin2mx}{2m} \right)\Big{|}_{0}^\pi,&m=n\end{cases}\\&=\begin{cases}0,&m\neq n\\ \pi,&m=n\end{cases}\end{align}$
同理可证,对于任意$m=1,2,\dots;n-=0,1,2,\dots$,
$\displaystyle\int_{-\pi}^{\pi} \cos mx\cos nx \, \mathrm{d}x=\begin{cases}0,&m\neq n\\ \pi,&m=n\neq0\\2\pi,&m=n=0\end{cases}$
因此,这组函数是$[-\pi,\pi]$上的正交函数列,又由于积分的周期性,这列函数是任一长度为$2\pi$的区间上的正交函数列
<span style='display: inline-block; width: 0.8em; height: 0.8em; background: #000; margin-left: 0.5em; vertical-align: middle; box-shadow: 1px 1px 2px rgba(0,0,0,0.2); margin-left:90%'></span>

<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg.  </span>
利用==递推法==求积分:
1)$I_{n}=\displaystyle\int_{0}^{\frac{\pi}{2}} \sin^nx \, \mathrm{d}x$(事实上$I_{n}=\displaystyle\int_{0}^{\frac{\pi}{2}} \cos^nx \, \mathrm{d}x$)
2)$I(m,n)=\displaystyle\int_{0}^{\frac{\pi}{2}} \sin^mx\cos^nx \, \mathrm{d}x$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
1)
两个积分相等可以由换元$t=\frac{\pi}{2}-x$得到
$I_{n}=\displaystyle\int_{0}^{\frac{\pi}{1}} \sin^{n-1}x \, \mathrm{d}(-\cos x)=-\sin^{n-1}x\cos x\Big{|}_{0}^{\frac{\pi}{2}}+\displaystyle\int_{0}^{\frac{\pi}{2}} \cos x \, \mathrm{d}(\sin^{n-1}x)=(n-1)I_{n-2}-(n-1)I_{n}$
故$I_{n}=\frac{n-1}{n}I_{n-2}(n\geqslant2)$
因此:$I_{n}=\begin{cases} \frac{(n-1)!!}{n!!},&n=2k+1\\ \frac{(n-1)!!}{n!!} \frac{\pi}{2},&n=2k\end{cases}\quad k\in \mathbb{N}$
...
>[!note] 
>对于2),可归纳如下:对于$\displaystyle\int_{\alpha}^{\beta} \sin^mx\cos^nx \, \mathrm{d}x$形式的积分,当$m,n$有一个是奇数时,可以用换元积分法求出积分值;当$m,n$都是偶数时,一般只能通过三角函数的恒等变形,将三角函数的幂指数降低到1后解决.但是当$\alpha=0,\beta \frac{\pi}{2}$时,只要$m,n$中有一个是偶数,就可以用1)的递推公式求值.

<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg.  </span>
==万能公式==
若$p(x)$是不超过3次的多项式,那么就有$\displaystyle\int_{a}^{b} p(x) \, \mathrm{d}x=\frac{1}{6}\left[ p(a)+4p\left( \frac{1}{2}(a+b) \right)+p(b) \right](b-a)$
...
>**Chebyshev不等式** (积分形式)设$f(x),g(x)$为拟序的,即$\forall x_{1},x_{2}:(f(x_{1})-f(x_{2}))(g(x_{1})-g(x_{2}))\geqslant0$,则$\displaystyle\int_{a}^{b} f(x) \, \mathrm{d}x\displaystyle\int_{a}^{b} g(x) \, \mathrm{d}x\leqslant(b-a)\displaystyle\int_{a}^{b} f(x)g(x) \, \mathrm{d}x$
>(离散形式)数列$\{ a_{n} \},\{ b_{n} \}$称为拟序的,即$\forall i,j:(a_{i}-a_{j})(b_{i}-b_{j})\geqslant0$,则$\left( \sum\limits_{i=1}^{n}a_{i} \right)\left( \sum\limits_{i=1}^{n}b_{i} \right)\leqslant n\sum\limits_{i=1}^{n}a_{i}b_{i}$
>反之称为反序的,不等式反向

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>

**Schwarz不等式**
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311159832.png)
>[!note] 
>![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311200125.png)
...
**Young不等式**
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311202502.png)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311203077.png)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311203579.png)
...
![RECTIFY_IMG_20250527_204413.jpg](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505272045704.jpg)
...
>[!example] 
>==p积分==
>对于无穷积分$\displaystyle\int_{1}^{+\infty} \frac{1}{x^p} \, \mathrm{d}x$,
>当$p>1$时,收敛于$\frac{1}{p-1}$；当$p\leqslant1$时,发散
>
>对于瑕积分$\displaystyle\int_{0}^{1} \frac{1}{x^p} \, \mathrm{d}x$,
...
<span style='color:#434343;background-image: linear-gradient(45deg, #93a5cf 0%, #e4efe9 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'> eg. 1 </span>
讨论敛散性:
1)==$\displaystyle\int_{0}^{+\infty} \frac{\sin x}{x^p} \, \mathrm{d}x\quad(p>0)$的变形==
1.1)$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x} \, \mathrm{d}x$
1.2)$\displaystyle\int_{0}^{+\infty} \frac{\sin x}{x^p+\sin x} \, \mathrm{d}x$
2)==p积分的变形==
2.1)$\displaystyle\int_{0}^{ \frac{1}{e}} \frac{1}{x^p\ln x} \, \mathrm{d}x,p\in \mathbb{R}^+$
2.2)$\displaystyle\int_{0}^{1} \frac{1}{x^p}\sin \frac{1}{x} \, \mathrm{d}x,p<2$

3)$\displaystyle\int_{0}^{+\infty} \frac{x^{1-p}}{\lvert x-1 \rvert^{p+q}} \, \mathrm{d}x,(p,q\in \mathbb{R})$

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
1)$1^\circ$ 当$0<p\leqslant1$时,
由于$0\leqslant\lim_\limits{ x \to 0^+} \frac{\sin x}{x^p}\leqslant1$,故$x=0$不是瑕点
在$[1,+\infty)$上$\frac{1}{x^p}\searrow0(x\to +\infty)$,$\displaystyle\int_{1}^{A} \sin x \, \mathrm{d}x$显然有界,由Dirichlet判别法,$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x^p} \, \mathrm{d}x$收敛  
在$[1,+\infty)$上,$\left\lvert  \frac{\sin x}{x^p}  \right\rvert\geqslant \frac{\sin^2 x}{x^p}=\frac{1}{2x^p}-\frac{\cos2x}{2x^p}$
$\displaystyle\int_{1}^{+\infty} \frac{\cos2x}{2x^p} \, \mathrm{d}x$收敛,而$\displaystyle\int_{1}^{+\infty} \frac{1}{2x^p} \, \mathrm{d}x$发散,故$\displaystyle\int_{1}^{+\infty} \frac{\sin^2x}{x^p} \, \mathrm{d}x$发散 
由比较判别法得,$\displaystyle\int_{1}^{+\infty} \lvert \frac{\sin x}{x^p} \rvert \, \mathrm{d}x$发散  ,故$\displaystyle\int_{0}^{+\infty} \lvert \frac{\sin x}{x^p} \rvert \, \mathrm{d}x$
故原积分在$0<p\leqslant1$时条件收敛 
$2^\circ$ $p>1$,
此时$x=0$是瑕点 
分解,$\displaystyle\int_{0}^{+ \infty} \frac{\sin x}{x^p} \, \mathrm{d}x=\left( \displaystyle\int_{0}^{1} +\displaystyle\int_{1}^{+\infty} \right) \frac{\sin x}{x^p}  \, \mathrm{d}x \xlongequal{let}I_{1}+I_{2}$
对于$I_{2}$,有$\left\lvert  \frac{\sin x}{x^p}  \right\rvert\leqslant \frac{1}{x^p}$,故绝对收敛
对于$I_{1}$,由$\frac{\sin x}{x^p}\sim x^{1-p}(x\to0^+)$得$I_{1}$在$1-p>-1$即$1<p<2$时绝对收敛,在$1-p\leqslant-1$即$p\geqslant2$时发散 
故原积分在$1<p<2$时绝对收敛,在$p\geqslant2$时条件收敛

1.1)在$[1,+\infty)$上$\frac{1}{x}\searrow0(x\to +\infty)$,$\displaystyle\int_{1}^{A} \sin x \, \mathrm{d}x$显然有界,由Dirichlet判别法,$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x} \, \mathrm{d}x$收敛 
但在$[1,+\infty)$上,$\left\lvert  \frac{\sin x}{x}  \right\rvert\geqslant \frac{\sin^2 x}{x}=\frac{1}{2x}-\frac{\cos2x}{2x}$
仿照上面的讨论得$\displaystyle\int_{1}^{+\infty} \frac{\cos2x}{2x} \, \mathrm{d}x$收敛,而$\displaystyle\int_{1}^{+\infty} \frac{1}{2x} \, \mathrm{d}x$发散,故$\displaystyle\int_{1}^{+\infty} \frac{\sin^2x}{x} \, \mathrm{d}x$发散 
由比较判别法得,$\displaystyle\int_{1}^{+\infty} \lvert \frac{\sin x}{x} \rvert \, \mathrm{d}x$发散 
故$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x} \, \mathrm{d}x$条件收敛 

1.2)
由于$0\leqslant\lim_\limits{ x \to 0^+} \frac{\sin x}{x^p+\sin x}\leqslant1$,故$x=0$不是瑕点,因此只需要讨论$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x^p+\sin x} \, \mathrm{d}x$的敛散性即可
由于$\frac{\sin x}{x^p+\sin x}=\frac{\sin x}{x^p}- \frac{\sin^2x}{x^p(x^p+\sin x)}$
由1)得$\displaystyle\int_{1}^{+\infty} \frac{\sin x}{x^p} \, \mathrm{d}x$在$9<p\leqslant1$时条件收敛;$p>1$时绝对收敛
对于$\displaystyle\int_{1}^{+\infty} \frac{\sin^2 x}{x^p(x^p+\sin x)} \, \mathrm{d}x$,
当$0<p\leqslant \frac{1}{2}$时,由$\frac{\sin^2x}{x^p(x^p+\sin x)}\geqslant \frac{\sin^2 x}{x^p(x^p+1)}$且$\displaystyle\int_{1}^{+\infty} \frac{\sin^2 x}{x^p(x^p+1)} \, \mathrm{d}x$发散,故积分发散
当$p>\frac{1}{2}$时,由$\frac{\sin^2x}{x^p(x^p+\sin x)}\leqslant \frac{1}{x^p(x^p-1)}\sim \frac{1}{x^{2p}}(x\to +\infty)$且$\displaystyle\int_{1}^{+\infty} \frac{1}{x^{2p}} \, \mathrm{d}x$收敛,故积分绝对收敛
因此,当$0<p\leqslant \frac{1}{2}$时发散;$\frac{1}{2}<p\leqslant1$时条件收敛;$p>1$时绝对收敛
...
2)p积分的敛散性直接由定义即可证明
2.1)$x=0$是该被积函数的唯一瑕点
当$0<p<1$时,取$q=\frac{1+p}{2}\in(p,1)$,则$\lim_\limits{ x \to 0^+} \frac{x^q}{x^p\lvert \ln x \rvert}=0$,由Cauchy判别法极限形式,原积分收敛
类似的,当$p>1$时,取$q=\frac{1+p}{2}\in(p,1)$,则$\lim_\limits{ x \to 0^+} \frac{x^q}{x^p\lvert \ln x \rvert}=+\infty$,由Cauchy判别法极限形式,原积分发散
当$p=1$时,直接使用Newton-Leibniz公式,有$\displaystyle\int_{0}^{\frac{1}{e}} \frac{1}{x\ln x} \, \mathrm{d}x=\lim_\limits{ \eta \to 0^+}\ln \lvert \ln x \rvert\Big{|}_{\eta}^{\frac{1}{e}}=-\infty$
因此,$0<p<1$时,收敛;$p\geqslant1$时,发散

2.2)$x=0$是该被积函数的唯一瑕点
令$f(x)=x^{2-p},g(x)=\frac{1}{x^2}\sin \frac{1}{x}$
$p<2$时,$\lim_\limits{ x \to 0^+ }f(x)=\lim_\limits{ x \to 0^+}x^{2-p}=0$,且$f(x)$显然单调
对于$\eta\in(0,1)$,有$\displaystyle\int_{\eta}^{1} g(x) \, \mathrm{d}x=\cos \frac{1}{x}\Big{|}_{\eta}^1$有界
由Dirichlet判别法,原积分收敛 
当$p<1$时,$\left\lvert  \frac{1}{x^p}\sin \frac{1}{x}  \right\rvert< \frac{1}{x^p}$,故绝对收敛 
当$1\leqslant p<2$时,用类似于1)的方法可得条件收敛
...
#### 3.3.1 特殊定积分
==Dirichlet积分==
$\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+ \frac{1}{2} \right)x}{\sin \frac{x}{2}} \, \mathrm{d}x=\pi,\quad n=0,1,2,\dots$被积函数$D_{n}(x)$称为Dirichlet核

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
由于$\lim_\limits{ x \to 0^+}D_{n}(x)=\frac{2n+1}{2}$,故在$[0,\pi]$上无瑕点,显然是连续的,故常义可积
由于$2\sin \frac{x}{2}\left( \frac{1}{2}+\sum\limits_{k=1}^{n}\cos kx \right)=\sin\left( n+\frac{1}{2} \right)x$
于是,$\frac{1}{2}D_{n}(x)=\frac{1}{2}+\sum\limits_{k=1}^{n}\cos kx$
故易得$\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+ \frac{1}{2} \right)x}{\sin \frac{x}{2}} \, \mathrm{d}x=\pi,\quad n=0,1,2,\dots$

==Fejèr积分==
$\displaystyle\int_{0}^{\pi} \left( \frac{\sin \frac{nx}{2}}{\sin \frac{x}{2}} \right)^2 \, \mathrm{d}x=n\pi,\quad n=0,1,2,\dots$
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
$\begin{align}I&=\displaystyle\int_{0}^{\frac{\pi}{2}} \ln \sin x \, \mathrm{d}x \xlongequal{x=2t}2\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \sin2t \, \mathrm{d}t=2\displaystyle\int_{0}^{\frac{\pi}{4}} \ln (2\sin t\cos t) \, \mathrm{d}t\\&= \frac{\pi}{2}\ln2+2\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \sin t \, \mathrm{d}t+2\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos t \, \mathrm{d}t\end{align}$
又由于$2\displaystyle\int_{0}^{\frac{\pi}{4}} \ln \cos t \, \mathrm{d}t \xlongequal{t= \frac{\pi}{2}-u}2\displaystyle\int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \ln \sin t \, \mathrm{d}t$
故$I=\frac{\pi}{2}\ln2+2I$
因此$I=-\frac{\pi}{2}\ln 2$
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
易证该积分条件收敛
由Dirichlet常义积分,$\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+ \frac{1}{2} \right)x}{2\sin \frac{x}{2}} \, \mathrm{d}x=\frac{\pi}{2}$
考虑将分母替换为$x$后两者的误差:$f(x)=\frac{1}{x}-\frac{1}{2\sin \frac{x}{2}}=O(x)(x\to0)$
因此$f(x)$在$[0,\pi]$上无瑕点,故连续,故常义可积
由Riemann引理,$\lim_\limits{ n \to \infty }\displaystyle\int_{0}^{\pi} f(x)\sin\left( n+\frac{1}{2} \right)x\, \mathrm{d}x=0$
且$\lim_\limits{ n \to \infty }\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+\frac{1}{2} \right)x}{x} \, \mathrm{d}x=\lim_\limits{ n \to \infty }\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+\frac{1}{2} \right)x}{2\sin \frac{x}{2}} \, \mathrm{d}x=\frac{\pi}{2}$
故$\displaystyle\int_{0}^{\pi} \frac{\sin\left( n+\frac{1}{2} \right)x}{x} \, \mathrm{d}x \xlongequal{t=\left( n+\frac{1}{2} \right)x}\displaystyle\int_{0}^{\left( n+\frac{1}{2} \right)\pi} \frac{\sin t}{t} \, \mathrm{d}x$
两边令$n\to +\infty$,即证
<span style='display: inline-block; width: 0.8em; height: 0.8em; background: #000; margin-left: 0.5em; vertical-align: middle; box-shadow: 1px 1px 2px rgba(0,0,0,0.2); margin-left:90%'></span>
...
==Special积分==
$\displaystyle\int_{0}^{+ \infty} \frac{1}{1+x^a\sin^bx} \, \mathrm{d}x$($a>b$,$b$为正偶数)

==Gamma函数离散形式==
$I_{n}=\displaystyle\int_{0}^{+\infty} e^{-x}x^n \, \mathrm{d}x$($n$是非负整数)

<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Solution :</span>
$n=0$时,直接由定义有$I_{0}=\displaystyle\int_{0}^{+\infty} e^{-x} \, \mathrm{d}x=1$
$n\geqslant1$时,利用分部积分:
$I_{n}=(-e^{-x}x^n)\Big{|}_{0}^{+\infty}+\displaystyle\int_{0}^{+\infty} e^{-x}x^{n-1} \, \mathrm{d}x=nI_{n-1}$
因此,$n\geqslant1$时,$I_{n}n!$

### 3.4 其它问题
#### 3.4.1 平方可积
> <span style='background-image: linear-gradient(120deg, #89f7fe 0%, #66a6ff 100%);font-size:1.5em;font-style:normal;padding:2px;font-family:方正龙爪简体;font-weight:bold;border-radius:0 10px 0 0;box-shadow: 2px 2px 5px 3px #ccc;'>「Definition:平方可积」</span>
> 若$\displaystyle\int_{a}^{+\infty} f^2(x) \, \mathrm{d}x$收敛,则称$f(x)$在$[a,+\infty)$上平方可积.
> 瑕积分可类似定义
> ==🖇️Properties：== 
> 1)平方可积与可积互不蕴含
> 2)对于无穷积分,平方可积与绝对可积互不蕴含
> 3)对于瑕积分,平方可积一定绝对可积,反之不真
...
1)
平方可积不保证可积,如$f(x)=\frac{1}{x},\quad x\in(1,+\infty)$
可积不保证平方可积,如$f(x)=\frac{\sin x}{\sqrt{ x }},\quad x\in(1,+\infty)$
2)
平方可积不保证绝对可积,如$f(x)=\frac{\sin x}{x},\quad x\in(1,+\infty)$
绝对可积不保证平方可积,如$f(x)=\begin{cases}n,&x\in\bigcup \limits_{n=2}^\infty\left[ n,n+\frac{1}{n^3} \right]\\0,&\text{other}\end{cases}\quad x\in(1,+\infty)$,或$f(x)=\frac{x}{1+x^6\sin^2x}\quad x\in(0,+\infty)$
3)
平方可积保证绝对可积,因为$\lvert f(x) \rvert\leqslant \frac{1}{2}[1+f^2(x)]$
绝对可积不保证平方可积,如$f(x)=\frac{1}{\sqrt{ x }},\quad x\in[0,1]$

#### 3.4.2 收敛无穷积分的被积函数在无穷远处的性质
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Counterpart-Example :</span>
$\displaystyle\int_{0}^{+\infty} \frac{x}{1+x^6\sin^2x} \, \mathrm{d}x$
<img src="https://locus622.oss-cn-beijing.aliyuncs.com/img/202505272147967.png" style="width:50%"/>

这个例子说明,无穷积分收敛时,$f(+\infty)$不仅可以不存在,而且可以有$\varlimsup\limits_{x\to +\infty}f(x)=+\infty$
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof 1:</span>
反证,如果$\lim_\limits{ x \to +\infty }f(x)\neq0$
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof 2:</span>
由$f(x)$在$[a,+\infty)$上一致连续,有:$\forall\varepsilon>0,\exists\delta>0,\forall x',x''\in[a,+\infty)(|x'-x''|<\delta):|f(x')-f(x'')|<\varepsilon$
由$\displaystyle\int_{a}^{+\infty} f(x) \, \mathrm{d}x$收敛得,对于上述的$\frac{\varepsilon\delta}{2},\exists A_{0}\geqslant a,\forall A,A''>A_{0}:|\displaystyle\int_{A}^{A''} f(x) \, \mathrm{d}x|< \frac{\varepsilon\delta}{2}$
令$A''=A+ \frac{1}{2}\delta$,有$|\displaystyle\int_{A}^{A+ \frac{1}{2}\delta} f(x) \, \mathrm{d}x|< \frac{\varepsilon\delta}{2}$
由积分第一中值定理,$\exists \xi\in\left( A,A+ \frac{1}{2}\delta \right),s.t.|\displaystyle\int_{A}^{A+\frac{1}{2}\delta} f(x) \, \mathrm{d}x|=|f(\xi)| \frac{1}{2}\delta$
由上述两式得$|f(\xi)|<\varepsilon$
而$|f(A)|\leqslant|f(A)-f(\xi)|+|f(\xi)|<2\varepsilon$
综上,$\forall\varepsilon>0,\exists A_{0}\geqslant a,\forall A>A_{0}:|f(A)|<\varepsilon$,即$\lim_\limits{ x \to +\infty }f(x)=0$
证毕
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311208286.png)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311208535.png)
...
<span style='color:#434343;background-image: linear-gradient(-20deg, #e9defa 0%, #fbfcdb 100%);border-radius:7px;box-shadow: 1px 1px 30px 5px rgb(204, 204, 204, 0.52);font-family:Monotype Corsiva;font-size:2em;'>Proof :</span>
1)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311208972.png)
2)
![image.png](https://locus622.oss-cn-beijing.aliyuncs.com/img/202505311208012.png)
