...
\begin{definition}{Partially Ordered Set (Poset)} % 偏序集
    ...
    Specifically, \(\prec\) is called a \textbf{strict partial order} on \(P\) if it is irreflexive, antisymmetric, and transitive.
\end{definition}

\begin{definition}{Totally Ordered Set (Chain)} % 全序集
    A \textbf{totally ordered set} (or \textbf{chain}) is a set \(P\) together with a partial order \(\preceq\)
    that is connected (total), i.e., for any \(x, y \in P\), either \(x \preceq y\) or \(y \preceq x\).
\end{definition}

\begin{definition}{Well-Ordered Set} % 良序集
    A \textbf{well-ordered set} is a set \(P\) together with a totally ordered \(\preceq\) that is well-founded, 
    i.e., every nonempty subset of \(P\) has a least element.
\end{definition}

\begin{theorem}{Well-Ordering Theorem} % 良序定理
    Every set can be well-ordered.
\end{theorem}

\begin{note}
    % 良序定理是选择公理的一个等价表述, 也是 ZF 中无法证明的命题.
...
\begin{definition}{Von Neumann Ordinal} % 冯诺依曼序数
...
\end{definition}
\begin{remark}
    % 在 ZF 中, 上述定义可等价为: \(\alpha\) 是一个序数当且仅当 \(\alpha\) 是一个传递集且它的所有元素都是传递集.
    In ZF, the above definition is equivalent to: \(\alpha\) is an ordinal if and only if 
    \(\alpha\) is a transitive set and all of its elements are transitive sets.
    % 这是由于正则公理保证了集合的良基性
