...
// 导语：积分理论需要处理多元函数，这需要乘积空间上的测度
// 本节将 Lebesgue 积分从一维推广到高维，建立乘积测度的构造和 Fubini-Tonelli 定理

So far we have developed the Lebesgue integral on a single measure space $(X, cal(S), mu)$. To integrate functions of several variables — for instance, to define the double integral $integral_(bb(R)^2) f(x,y) dif x dif y$ — we need to construct a measure on the product space $X times Y$ from given measures on $X$ and $Y$. The *Fubini-Tonelli theorems* then allow us to compute such integrals by iterating one-dimensional integrals, justifying the familiar calculus technique of "repeated integration."

=== Product sigma-Algebras // 乘积 sigma-代数

// 乘积 σ-代数是包含所有可测矩形的最小 σ-代数
// 截面可测性是建立 Fubini 定理的基础工具

Let $(X, cal(S))$ and $(Y, cal(T))$ be measurable spaces.

#definition(name: "Product sigma-Algebra")[
  The *product sigma-algebra* of $cal(S)$ and $cal(T)$, denoted $cal(S) times.o cal(T)$, is the $sigma$-algebra on $X times Y$ generated by the collection of *measurable rectangles*:
  $
    {A times B : A in cal(S), B in cal(T)}.
  $
  The measurable space $(X times Y, cal(S) times.o cal(T))$ is called the *product measurable space*.
]

#definition(name: "Sections of a Set")[
  Let $E subset.eq X times Y$. For each $x in X$, the *$x$-section* of $E$ is
  $
    E_x = {y in Y : (x,y) in E}.
  $
  Similarly, for each $y in Y$, the *$y$-section* is $E^y = {x in X : (x,y) in E}$.
]

#lemma(name: "Measurability of Sections")[
  If $E in cal(S) times.o cal(T)$, then for every $x in X$, $E_x in cal(T)$, and for every $y in Y$, $E^y in cal(S)$.
]

#proof[
  Let $cal(R) = {E subset.eq X times Y : E_x in cal(T) "for all" x in X}$. We show $cal(R)$ is a $sigma$-algebra containing all measurable rectangles.

  *Step 1: Rectangles are in $cal(R)$.* If $E = A times B$, then $E_x = B$ if $x in A$ and $E_x = emptyset$ otherwise. In either case $E_x in cal(T)$.

  *Step 2: $cal(R)$ is closed under complement and countable union.* If $E in cal(R)$, then $(E^("c"))_x = (E_x)^("c") in cal(T)$. If $E_1, E_2, ... in cal(R)$, then $(union.big_n E_n)_x = union.big_n (E_n)_x in cal(T)$.

  Since $cal(R)$ is a $sigma$-algebra containing the generators of $cal(S) times.o cal(T)$, we have $cal(S) times.o cal(T) subset.eq cal(R)$. The proof for $E^y$ is symmetric.
]

#note[
  The section lemma is the bridge between product measurability and one-dimensional measurability. It ensures that when we "freeze" one variable, the resulting function of the remaining variable remains measurable — a crucial ingredient for defining iterated integrals.
]

=== Product Measures // 乘积测度

// 对 σ-有限测度空间，乘积测度由矩形上的值唯一确定

Let $(X, cal(S), mu)$ and $(Y, cal(T), nu)$ be $sigma$-finite measure spaces.

#definition(name: "Product Measure")[
  A *product measure* of $mu$ and $nu$ is a measure $mu times nu$ on $(X times Y, cal(S) times.o cal(T))$ satisfying
  $
    (mu times nu)(A times B) = mu(A) nu(B)
  $
  for all $A in cal(S)$, $B in cal(T)$. For $sigma$-finite measure spaces, such a measure exists and is unique.
]

#proof[
  Define $lambda$ on the semiring of measurable rectangles by $lambda(A times B) = mu(A) nu(B)$. One checks that $lambda$ is $sigma$-additive on rectangles. By the Carathéodory extension theorem, $lambda$ extends uniquely to a measure on $cal(S) times.o cal(T)$. The $sigma$-finiteness of $mu$ and $nu$ guarantees uniqueness of the extension.
]

#example[
  The Lebesgue measure $m_2$ on $bb(R)^2$ coincides with the product measure $m times m$, where $m$ is one-dimensional Lebesgue measure. In particular, for any rectangle $[a,b] times [c,d]$,
  $
    m_2([a,b] times [c,d]) = (b-a)(d-c) = m([a,b]) dot m([c,d]).
  $
  This justifies the usual formula for area and is consistent with the construction of Lebesgue measure on $bb(R)^n$ via outer measure.
]

=== Fubini-Tonelli Theorems // Fubini-Tonelli 定理

// 核心定理：积分次序交换的条件
// Tonelli（非负可测函数）和 Fubini（可积函数）一起给出了累次积分的完整理论

We now state the two central theorems of product integration. The first applies to non-negative measurable functions and requires no integrability assumption; the second extends the result to integrable (possibly signed) functions.

#theorem(name: "Tonelli's Theorem")[
  Let $(X, cal(S), mu)$ and $(Y, cal(T), nu)$ be $sigma$-finite measure spaces, and let $f: X times Y -> [0, infinity]$ be $cal(S) times.o cal(T)$-measurable. Then:
  1. For each $x in X$, the function $y |-> f(x,y)$ is $cal(T)$-measurable, and $x |-> integral_Y f(x,y) dif nu(y)$ is $cal(S)$-measurable (similarly with roles of $X$ and $Y$ swapped).
  2. The iterated integrals equal the product integral:
    $
      integral_(X times Y) f dif (mu times nu) = integral_X (integral_Y f(x,y) dif nu(y)) dif mu(x) = integral_Y (integral_X f(x,y) dif mu(x)) dif nu(y).
    $
]

#proof[
  We prove the theorem in three standard steps.

  *Step 1: Indicator functions.* For $E in cal(S) times.o cal(T)$, define $f = chi_E$. By the section lemma, $chi_E(x, dot) = chi_(E_x)$ is $cal(T)$-measurable, and
  $
    integral_Y chi_E(x, y) dif nu(y) = nu(E_x).
  $
  The function $x |-> nu(E_x)$ is $cal(S)$-measurable (a consequence of the monotone class lemma or a direct $pi$-$lambda$ argument). Moreover,
  $
    integral_X nu(E_x) dif mu(x) = (mu times nu)(E) = integral_(X times Y) chi_E dif (mu times nu).
  $
  This establishes the theorem for indicator functions.

  *Step 2: Simple functions.* By linearity, the result holds for all non-negative simple functions $s$.

  *Step 3: Non-negative measurable functions.* Let $f$ be non-negative measurable. Choose an increasing sequence of simple functions $s_n arrow.t f$. By the Monotone Convergence Theorem applied at each level,
  $
    integral_(X times Y) f dif (mu times nu) = lim_(n->infinity) integral_(X times Y) s_n dif (mu times nu)
  $
  and similarly for the iterated integrals. The measurability of the section functions is preserved under pointwise limits, completing the proof.
]

#note[
  *Why no integrability condition?* Tonelli's theorem applies to all non-negative measurable functions — the integrals may be infinite, but the equality always holds. This makes it an extremely versatile tool: to justify swapping the order of integration, it suffices to check non-negativity, not absolute integrability.
]

#theorem(name: "Fubini's Theorem")[
  Let $(X, cal(S), mu)$ and $(Y, cal(T), nu)$ be $sigma$-finite measure spaces, and let $f: X times Y -> overline(bb(R))$ be $cal(S) times.o cal(T)$-measurable and $mu times nu$-integrable, i.e., $f in L^1 (X times Y, mu times nu)$. Then:
  1. For $mu$-almost every $x in X$, the section $f_x: y |-> f(x,y)$ is $nu$-integrable, and for $nu$-almost every $y in Y$, $f^y: x |-> f(x,y)$ is $mu$-integrable.
  2. The functions $x |-> integral_Y f(x,y) dif nu(y)$ (defined a.e.) and $y |-> integral_X f(x,y) dif mu(x)$ (defined a.e.) belong to $L^1 (X, mu)$ and $L^1 (Y, nu)$ respectively.
  3. The product integral equals both iterated integrals:
    $
      integral_(X times Y) f dif (mu times nu) = integral_X (integral_Y f(x,y) dif nu(y)) dif mu(x) = integral_Y (integral_X f(x,y) dif mu(x)) dif nu(y).
    $
]
...
#proof[
  Decompose $f = f^+ - f^-$ into positive and negative parts. Both $f^+$ and $f^-$ are non-negative measurable, and by integrability of $f$, $integral f^+ < infinity$ and $integral f^- < infinity$.

  Apply Tonelli's theorem to $f^+$ and $f^-$ separately:
  $
...
// 例子：经典高斯积分计算
// 通过 Fubini 将二维积分化为一维累次积分，再用极坐标换元

#example[
  *Gaussian integral*. Compute $I = integral_(-infinity)^infinity e^(-x^2) dif x$. Consider the two-dimensional integral
  $
    I^2 = (integral_(-infinity)^infinity e^(-x^2) dif x) (integral_(-infinity)^infinity e^(-y^2) dif y) = integral_(bb(R)^2) e^(-(x^2 + y^2)) dif x dif y.
  $
  By Tonelli's theorem, the double integral equals the iterated integral. Switching to polar coordinates $x = r cos theta$, $y = r sin theta$:
  $
    I^2 = integral_0^(2 pi) integral_0^infinity e^(-r^2) r dif r dif theta = 2 pi dot [-(1 / 2) e^(-r^2)]_(0)^infinity = pi.
  $
  Therefore $I = sqrt(pi)$. This derivation uses Tonelli (not Fubini) because the integrand is non-negative — no absolute integrability check is needed.
]

// 反例：当绝对可积条件不满足时，Fubini 定理失效
...
#proposition(name: "Sections of Measurable Functions")[
  Let $(X, cal(S))$ and $(Y, cal(T))$ be measurable spaces, and let $f: X times Y -> overline(bb(R))$ be $cal(S) times.o cal(T)$-measurable. Then:
  - For each $x in X$, the section $f_x: y |-> f(x,y)$ is $cal(T)$-measurable.
  - For each $y in Y$, the section $f^y: x |-> f(x,y)$ is $cal(S)$-measurable.
]

#proof[
  For any $alpha in bb(R)$, ${y : f_x(y) > alpha} = {y : f(x,y) > alpha} = ({(x',y') : f(x',y') > alpha})_x$.
  Since ${f > alpha} in cal(S) times.o cal(T)$ by measurability of $f$, its $x$-section is in $cal(T)$ by the section lemma. The same argument applies to $f^y$.
]

=== Layer Cake Formula // 层蛋糕公式

// Fubini 定理的一个重要应用：将函数积分转化为测度分布的积分

As an application of Fubini's theorem, we derive the *layer cake representation* of a non-negative measurable function, a useful tool in $L^p$ theory and probability.

#theorem(name: "Layer Cake Formula")[
  Let $(X, cal(S), mu)$ be a $sigma$-finite measure space and $f: X -> [0, infinity]$ be measurable. Then for any $p >= 1$,
  $
    integral_X f^p dif mu = p integral_0^infinity t^(p-1) mu({x : f(x) > t}) dif t.
  $
  In particular, for $p = 1$:
  $
    integral_X f dif mu = integral_0^infinity mu({f > t}) dif t.
  $
]

#proof[
  Write $f^p(x) = p integral_0^(f(x)) t^(p-1) dif t = p integral_0^infinity t^(p-1) chi_({f > t})(x) dif t$.
  By Tonelli's theorem applied to the non-negative function $(x,t) |-> t^(p-1) chi_({f > t})(x)$ on $X times (0, infinity)$:
  $
    integral_X f^p dif mu = integral_X (p integral_0^infinity t^(p-1) chi_({f > t})(x) dif t) dif mu(x) = p integral_0^infinity t^(p-1) mu({f > t}) dif t.
  $
]

#note[
  The layer cake formula expresses an integral with respect to $mu$ as an integral with respect to Lebesgue measure on $(0, infinity)$. It is particularly useful for:
  - Estimating $L^p$ norms in terms of distribution functions,
  - Proving interpolation inequalities (e.g., the Marcinkiewicz interpolation theorem),
  - Relating $L^p$ spaces to weak $L^p$ (Lorentz) spaces.
]

#v(0.5em)
// 过渡段：乘积测度与 Fubini 定理完成了积分理论的最后一块拼图
// 下一章开始研究微分理论：函数的可微性与积分的逆运算

With the Fubini-Tonelli theorems, we have completed the core theory of Lebesgue integration. Product measures allow us to integrate functions of several variables, and the interchange of iterated integrals provides a powerful computational tool. In the next chapter, we turn to the other side of analysis — differentiation — and study the relationship between a function and its derivative from the measure-theoretic perspective.
...
#tex-table(
  ...
  (
    "$mu$ = counting measure on $bb(N)$",
    "$ell^p subset.eq ell^q$ for $p < q$",
    "$|a_n|^q <= |a_n|^p$ when $|a_n| <= 1$",
  ),
